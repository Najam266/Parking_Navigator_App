    ...
    public partial class Addnewuser : Form
    {
        ...
        private void add_further()
        {
            ...
            using (SQLiteConnection con1 = new SQLiteConnection(connectionstring))
            {
                ...
                using (SQLiteCommand cmd = new SQLiteCommand(query3, con1))
                {
                    cmd.Parameters.AddWithValue("@ID", ID);
                    cmd.Parameters.AddWithValue("@name", name);
                    cmd.Parameters.AddWithValue("@registration", registration);
        ...
        private bool parkingcheck()
        {
            ...
            using (SQLiteConnection con2 = new SQLiteConnection(connectionstring))
            {
                ...
                string checkQuery = "SELECT count(*) FROM Parking WHERE UserID = @ID AND Name = @name AND RegistrationNo = @registration";
                using (SQLiteCommand checkCmd = new SQLiteCommand(checkQuery, con2))
                {
                    checkCmd.Parameters.AddWithValue("@ID", ID);
                    checkCmd.Parameters.AddWithValue("@name", name);
        ...
        private void fillslot(string FLOOR)
        {
            ...
            if (checkings(f))
            {
                ...
                using (SQLiteConnection con = new SQLiteConnection(connectionstring))
                {
                    ...
                    string query2 = $"UPDATE {FLOOR} SET User_id = @ID, PlateNo = @registration, Status = @mess WHERE Slot = @slot";
                    using (SQLiteCommand cmd = new SQLiteCommand(query2, con))
                    {
                        cmd.Parameters.AddWithValue("@ID", ID);
                        cmd.Parameters.AddWithValue("@registration", registration);
