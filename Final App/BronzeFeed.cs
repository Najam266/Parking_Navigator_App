...
namespace APP
{
    public partial class BronzeFeed : Form
    {
        ...
        private void BronzeFeed_Load(object sender, EventArgs e)
        {
            showb("Bronze");
        }
        private void showb(string typname)
        {
            using (SQLiteConnection con = new SQLiteConnection(connectionstring))
            {
                ...
                string query2 = @"SELECT count(f.Registration_No) FROM Feed f inner join History h on f.UserID=h.UserId inner join Members m on h.UserID=m.UserId  where m.Type=@typname";
