    ...
    public partial class Profitbymembers : Form
    {
        ...
        private void showS(string typname)
        {
            using (SQLiteConnection con = new SQLiteConnection(connectionstring))
            {
                ...
                string query = @"SELECT h.Name,m.Type,h.Price from History h inner join Members m on h.UserID=m.UserId where m.Type=@typname";
                string query2= @"SELECT SUM(h.Price) AS TotalPrice FROM History h inner join Members m on h.UserID=m.UserId where m.Type=@typname";
        ...
        private void Profitbymembers_Load(object sender, EventArgs e)
        {
